    ...
    return {
        ...
        initPieCharts: function () {
        	...
        	$.ajax({
			    ...
			    async: true, //Math.random() * 10
			    type: "post",   //Math.floor() + 1   parseInt(re[i].dingdao)	
			  success:function(re) {		   		
			    $.each(re,function(i){				 
					 // alert("/ "+re[i].dingdao);
					  data[i] = {
							  label: "年级:" +re[i].grade+" 出勤率:"+re[i].iattendance,
							  data: parseInt(re[i].dingdao)
					  }						  
        	...
        	$.ajax({
			  ...
			  success:function(re) {		   		
			    $.each(re,function(i){				 
			    	 data[i] = {
							  label: "年级:" +re[i].grade+" 应到比率",
							  data: parseInt(re[i].yingdao)
					  }					  		   	
			    });
