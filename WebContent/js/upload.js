(function($) {
	...
	$(function() {
		...
		uploader = WebUploader.create({
			...
			chunked : true,
			chunkSize : 512 * 1024,
			server : 'userinfo/updateuserpic.action',
			method : 'POST',
			// runtimeOrder: 'flash',
		...
		// 添加“添加下一个”模型的按钮，
		/*
		 * uploader.addButton({ id: '#addModel', label: '添加下一个' });
		 */
			...
			if (state === 'ready') {
				...
				if (stats.uploadFailNum) {
					text = '已成功上传'
							...
							+ '文件，'
							+ stats.uploadFailNum
							+ '文件上传失败，<a class="retry" href="#">重新上传</a>失败或<a class="ignore" href="#">忽略</a>'
				}
		...
		function setState(val) {
			...
			case 'finish':
				stats = uploader.getStats();
				if (stats.successNum) {
					// document.getElementById("shareUrl").style.visibility =
					// 'visible';
					// createQrcode();
					setTimeout(function() {
						parent.location.replace("permanage.html");
					}, 1200);
